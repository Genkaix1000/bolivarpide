...
import { OrderWarningGlyph } from "./orderStatusIcons";
...
export function OrderTrackingSheet({ view }: { view: OrderTrackingView }) {
  ...
  return (
    <>
      <div
        className={cn(
          "relative z-30 max-h-[46vh] shrink-0 overflow-y-auto rounded-t-[28px] px-4 pb-6 pt-4 shadow-2xl",
      ...
      >
        ...
        {view.status === "rejected" ? (
          <div className="mb-3 flex items-start gap-3 rounded-2xl border border-red-200 bg-red-50 p-3.5 dark:border-red-900/40 dark:bg-red-950/40">
            <OrderWarningGlyph size={22} className="shrink-0 text-red-500" />
            <div>
          ...
          <>
            ...
            {view.map?.fulfillmentType === "delivery" && view.map.destination?.label ? (
              <p className="mt-1.5 flex items-start gap-1 text-[11px] text-stone-600 dark:text-stone-400">
                <MapPin weight="fill" size={14} className="mt-0.5 shrink-0 text-[#9a0002]" />
              ...
              <p className="mt-1.5 flex items-center gap-1 text-[11px] font-semibold text-[#9a0002] dark:text-red-400">
                <Storefront weight="regular" size={14} />
        ...
        <div className="flex items-center gap-3 border-t border-[#e8e0d6] pt-3.5 dark:border-stone-800">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#f5f1eb] text-xs font-bold text-stone-700 ring-1 ring-black/5 dark:bg-stone-800 dark:text-stone-300">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-0.5 flex items-center gap-1 truncate text-[11px] font-medium text-stone-500 dark:text-stone-400">
              <Star weight="fill" size={13} className="text-amber-500" />
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            <a
              ...
              className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-full bg-[#f5f1eb] text-stone-700 shadow-sm transition-colors hover:text-[#9a0002] dark:bg-stone-800 dark:text-stone-200 dark:hover:text-red-400"
              title="Abrir en Google Maps"
              aria-label="Abrir ubicación en Google Maps"
            >
              <MapTrifold weight="regular" size={18} />
            </a>

            {tel ? (
              <a
                href={`tel:${tel}`}
                className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-full bg-[#f5f1eb] text-[#9a0002] shadow-sm transition-colors hover:bg-[#ede4d9] dark:bg-stone-800 dark:text-white"
                aria-label="Llamar al local"
                title="Llamar al local"
              >
                <Phone weight="regular" size={18} />
              </a>
            ) : null}
          </div>
        </div>
