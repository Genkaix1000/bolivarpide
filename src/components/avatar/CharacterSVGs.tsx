...
/**
 * Unified Classic Icon / Emoji / Initials SVG Renderer
 * Shares the exact same 160x160 viewBox, disc (cx=80, cy=88, r=54) and alignment!
 */
export function ClassicIconRenderer({
  avatar,
  className = "w-full h-full",
}: {
  avatar: { type: "symbol" | "emoji" | "initials"; value: string; gradientId: string };
  className?: string;
}) {
  ...
  return (
    <svg viewBox="0 0 160 160" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} style={{ overflow: "visible" }}>
      ...
      {/* Material Symbol */}
      {avatar.type === "symbol" && (
        <foreignObject x="38" y="46" width="84" height="84">
          <div className="w-full h-full flex items-center justify-center text-white select-none">
            <span
              className="material-symbols-rounded text-white select-none"
