...
export default function Navbar({
  ...
  const { activeOrder, cart, pendingOrder } = useCart();
  ...
  const cartBlockingFloat = cartItemCount(cart.lines) > 0 || !!pendingOrder;

  const canQuickContinue = !!rememberedAccount && readGuestMode();
  const showLoginFloat =
    !isAuthLoading && !isAuthenticated && !cartBlockingFloat && !showLoginSheet && !rememberedAccount;
    ...
    <div className="fixed inset-x-0 top-0 z-50">
      ...
      >
        {!isAuthLoading && !isAuthenticated ? (
          ...
          <nav className="mx-auto flex h-full w-full max-w-[760px] items-center gap-2 px-4">
            <button
              type="button"
              onClick={onSearchFocus}
              aria-label="Buscar"
              className={cn(
                "flex h-11 min-w-0 flex-1 cursor-pointer items-center gap-2.5 rounded-2xl border border-[#e8e0d6] bg-white px-3.5 text-left shadow-sm transition-colors",
            ...
            <button
              ...
              className={cn(
                ...
                "border border-[#e8e0d6] bg-white text-stone-600 shadow-sm transition-colors",
                "hover:border-[#9a0002]/40 hover:text-[#9a0002] dark:border-[#3d3732] dark:bg-[#1c1917] dark:text-stone-300",
              )}
            >
      ...
      <AccountSheet
        open={showUserMenu && isAuthenticated}
        ...
        appearanceControl={<ThemeToggleNavBtn className="h-8 w-8" clipId="nav-theme-sheet" />}
      />
    </div>
  );
}
